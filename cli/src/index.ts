...
import {
  ...
  resolveGameRoot,
  GAMES_PARENT_DIR,
  listExistingGameSlugs,
} from "./game-root.js";

const program = new Command();

async function chooseGameSlug(workspaceRoot: string): Promise<string> {
...
`
  ...
  .action(async (gameArg: string | undefined, opts: { port?: string; game?: string }) => {
...
program
  .command("preview")
  .description(
    "单独启动本地预览（可与 game 分终端运行；重启预览只需 Ctrl+C 后再 preview，不结束对话）"
  )
  .argument(
    "[workspace]",
    "工作区根目录（其下为 games/<游戏名>）",
    "."
  )
  .option("-p, --port <port>", "端口", "4321")
  .option(
    "-g, --game <slug>",
    `游戏子目录名（预览 ./${GAMES_PARENT_DIR}/<slug>）`,
    "default"
  )
  .addHelpText(
    "after",
...
`
  ...
  .action(async (workspace: string, opts: { port?: string; game?: string }) => {
    ...
    if (await probeSparkPreviewShell(port)) {
      ...
      console.log(
        chalk.dim(
          "  若需为本游戏单独起服务，请先停掉占用该端口的进程，或使用：spark preview -p 4322\n"
        )
      );
      process.exit(0);
    }
    ...
    console.log(
      chalk.dim(
        "  Ctrl+C 停止本预览；另一终端的 spark game 可继续使用（会先探测本服务）\n"
      )
    );
    const shutdown = () => {
      srv.close();
