...
async function chooseGameSlugForPush(workspaceRoot) {
    const games = listExistingGameSlugs(workspaceRoot);
    if (games.length === 0) {
        return "default";
    }
...
program
    ...
    .option("-g, --game <slug>", `游戏子目录名（推送 ./${GAMES_PARENT_DIR}/<slug>）`)
...
`)
    ...
    const workspaceRoot = rawTarget === "." || rawTarget.includes(path.sep)
        ? path.resolve(process.cwd(), rawTarget)
        : process.cwd();
    const pushOne = async (gameSlug, remoteSlug) => {
        const gameDir = resolveGameRoot(workspaceRoot, gameSlug);
        const r = await pushSparkSnapshot(config, gameDir, (remoteSlug || gameSlug).trim() || gameSlug);
        if (r.ok) {
            console.log(chalk.green(`✓ 已推送游戏 ${gameSlug} → 云端 slug=${(remoteSlug || gameSlug).trim() || gameSlug}`));
            return true;
        }
