...
/** 规范化游戏子目录名，禁止路径穿越 */
export function normalizeGameSlug(raw) {
    const t = raw
        .trim()
        .replace(/[/\\]+/g, "-")
        .replace(/\s+/g, "-");
...
`;
function indexHtmlNeedsSeed(gameRoot) {
...
export function ensureGameRoot(gameRoot) {
    fs.mkdirSync(gameRoot, { recursive: true });
    if (indexHtmlNeedsSeed(gameRoot)) {
        fs.writeFileSync(path.join(gameRoot, "index.html"), MINIMAL_INDEX_HTML, "utf-8");
    }
}
