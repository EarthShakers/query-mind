...
export default function Page() {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      {/* Sidebar */}
      <aside className="w-72 shrink-0 border-r border-slate-200 bg-white/80 backdrop-blur flex flex-col">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className="shrink-0 px-6 py-4 border-b border-slate-200 bg-white/60 backdrop-blur">
          ...
          {messages.map((m, idx) => {
            ...
            if (m.role === "user") {
              return (
                <div key={m.id} className="flex justify-end">
                  <div className="max-w-[70%] px-4 py-3 rounded-2xl rounded-tr-sm bg-indigo-500 text-white text-sm shadow-sm">
                    {m.content}
                  </div>
                </div>
              );
            ...
            return (
              <div key={m.id} className="flex justify-start">
                <div className="max-w-[85%] px-4 py-3 rounded-2xl rounded-tl-sm bg-white border border-slate-200 shadow-sm">
                  {m.content && (
        ...
        {/* Input */}
        <div className="shrink-0 px-6 py-4 border-t border-slate-200 bg-white/60 backdrop-blur">
          <form
            onSubmit={(e) => {
          ...
          >
            <input
              ...
              placeholder="输入你想查询的数据，例如：各部门本月支出明细..."
              className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent shadow-sm placeholder:text-slate-300"
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="px-5 py-3 bg-indigo-500 text-white text-sm font-medium rounded-xl hover:bg-indigo-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm"
            >
              发送
            </button>
          </form>
