...
const SelfQuerySchema = z.object({
  ...
  filterTitle: z
    ...
    .optional()
    .describe(
      "仅当用户明确指定要查某份文档/文件时填写其标题或文件名；否则必须为 null"
    ),
});
 ...
 */
export async function parseSelfQuery(
  userMessage: string
): Promise<SelfQueryResult> {
  if (!userMessage?.trim()) {
    return { query: "" };
  }
  ...
  try {
    ...
    })) as z.infer<typeof SelfQuerySchema>;

    const query = result?.query?.trim() || userMessage.trim();
    const rawFilter = result?.filterTitle;
    let filterTitle =
      typeof rawFilter === "string" &&
      rawFilter.trim() &&
      rawFilter.trim().toLowerCase() !== "null"
        ? rawFilter.trim()
        : undefined;

    // 启发式：仅当用户明确用「里」「中」等限定文档时才用 filterTitle，避免「李卓超的帅照」被误解析
