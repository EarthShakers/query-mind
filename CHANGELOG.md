...
- **评估结果持久化**：CLI 评估完成后自动写入 Supabase `eval_runs` 表，4 个指标独立列，方便趋势查询
- **评估 API**：`GET /api/eval-runs?limit=30` 返回最近 N 条评估记录
- **导航入口**：Chat 侧边栏和知识库页面顶栏新增「评估看板」链接
- **GitHub Actions 自动化**：定时 Cron 每天 8:00 / 20:00 自动运行 E2E 评估，结果自动入库
- **run_mode 修复**：E2E 模式下 `run_mode` 显式传参，不再依赖 `datasetPath` 字符串推断
