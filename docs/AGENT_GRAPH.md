...
const graph = new StateGraph(AgentState)
  ...
  .addConditionalEdges("planning", (state) => {
    ...
    return (state.plan?.sub_tasks?.length ?? 0) > 0
      ? "execute"
      : "synthesize";
  })
...
用户在输入框旁手动开启「深度思考」按钮 → `deepThink=true` 随请求发送 → `route.ts` 走 `createAgentStreamResponse`。默认关闭时走 streamText。Report 模式下忽略深度思考开关。

## 与 streamText 的区别

| 对比项 | streamText（默认） | LangGraph Agent（深度思考） |
|--------|----------------------|---------------------------|
| 触发方式 | 默认 | 用户手动开启「深度思考」按钮 |
