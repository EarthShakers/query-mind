...
| 指标 | 最新分数 | 说明 |
|------|----------|------|
| Faithfulness | 稳定 | 生成答案基于上下文，幻觉率低 |
| Answer Relevancy | 稳定 | 答案与问题匹配度高 |
| Context Precision | **17.4%** | **不稳定，需重点优化**——检索到的相关文档未排在前列，加权 precision@k 偏低 |
| Context Recall | 稳定 | 参考答案关键信息被检索覆盖 |

> **优化方向**：Context Precision 低表明检索排序质量差，相关 chunk 被噪声 chunk 稀释。可从以下方向改进：
> - BM25 + 向量混合检索（关键词精确匹配 + 语义相似度互补）
> - Rerank 模型强制启用（Cohere / Cross-Encoder）
> - Enrich 预检索（意图路由 + 结构化 filter 提取，减少无关检索）
> - 扩充评估样本量，排查低分 case
