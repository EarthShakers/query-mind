...
| 工作项                | 重要度 | 状态 | 具体内容                                                                       |
| --------------------- | ------ | ---- | ------------------------------------------------------------------------------ |
| **1. 指标体系搭建**   | ★★★★★  | ❌   | 定义检索质量 + 生成质量指标，与 RAG Pipeline 各阶段对齐                        |
| **2. RAGAS 集成**     | ★★★★★  | ❌   | 接入 RAGAS 框架，支持 Context Precision/Recall、Faithfulness、Answer Relevancy |
| **3. 评估数据集**     | ★★★★★  | ❌   | 构建 question/answer/contexts/ground_truth 格式测试集，可手工或 RAGAS 自动生成 |
| **4. 自动化基准测试** | ★★★★☆  | ❌   | 单次/批量评估脚本，支持按 space、按文档抽样，输出多维度评分报告                |
...
| 指标类型 | 指标名              | 说明                               |
| -------- | ------------------- | ---------------------------------- |
| 检索质量 | Context Precision   | 检索文档与问题的相关性             |
| 检索质量 | Context Recall      | 检索信息的完整性，是否覆盖关键信息 |
| 生成质量 | Faithfulness        | 答案是否基于检索上下文，检测幻觉   |
| 生成质量 | Answer Relevancy    | 答案与问题的匹配程度               |
| 可选     | Factual Correctness | 答案事实准确性（需 ground_truth）  |

**下一步**：RAGAS 集成、评估数据集构建、自动化基准测试。
